...
exports.isTrackedHasuraEventHandlerConfig = (eventHandlerConfig) => {
...
const updateEventTriggerMetaV2 = (moduleConfig, eventHandlerConfigs) => {
    ...
    const tablesMeta = fs_1.readFileSync(tablesYamlPath, utf8);
    const tableEntries = js_yaml_1.load(tablesMeta);
    lodash_1.orderBy(eventHandlerConfigs, (x) => x.triggerName).forEach((config) => {
        ...
        matchingTable.event_triggers = event_triggers_1.mergeEventHandlerConfig(config, moduleConfig, defaultRetryConfig, matchingTable);
    });
    const yamlString = js_yaml_1.dump(tableEntries);
    fs_1.writeFileSync(tablesYamlPath, yamlString, utf8);
};
const updateEventTriggerMetaV3 = (moduleConfig, eventHandlerConfigs) => {
    ...
    eventHandlerConfigs.forEach((config) => {
        ...
        const tableMeta = fs_1.readFileSync(tableYamlPath, utf8);
        const tableEntry = js_yaml_1.load(tableMeta);
        tableEntry.event_triggers = event_triggers_1.mergeEventHandlerConfig(config, moduleConfig, defaultRetryConfig, tableEntry);
        const yamlString = js_yaml_1.dump(tableEntry);
        fs_1.writeFileSync(tableYamlPath, yamlString, utf8);
    });
};
exports.updateEventTriggerMeta = (moduleConfig, eventHandlerConfigs) => {
    const { managedMetaDataConfig } = moduleConfig;
...
exports.updateScheduledEventTriggerMeta = (moduleConfig, scheduledEventHandlerConfigs) => {
    var _a, _b;
    const { managedMetaDataConfig } = moduleConfig;
    if (!managedMetaDataConfig) {
        throw new Error(MISSING_META_CONFIG);
    ...
    const cronTriggersMeta = fs_1.readFileSync(cronTriggersYamlPath, utf8);
    const cronEntries = ((_a = js_yaml_1.load(cronTriggersMeta)) !== null && _a !== void 0 ? _a : []);
    ...
    const yamlString = js_yaml_1.dump(newCronEntries);
    fs_1.writeFileSync(cronTriggersYamlPath, yamlString, utf8);
};
