...
let HasuraModule = HasuraModule_1 = class HasuraModule extends nestjs_modules_1.createConfigurableDynamicRootModule(hasura_constants_1.HASURA_MODULE_CONFIG, {
    providers: [
        {
    ...
    async onModuleInit() {
        ...
        if (this.hasuraModuleConfig.managedMetaDataConfig) {
            ...
            hasura_metadata_1.updateEventTriggerMeta(this.hasuraModuleConfig, trackedEventHandlerMeta
                .filter((x) => hasura_metadata_1.isTrackedHasuraEventHandlerConfig(x.meta))
                .map((x) => x.meta));
            if (trackedScheduledEventHandlerMeta.length) {
                hasura_metadata_1.updateScheduledEventTriggerMeta(this.hasuraModuleConfig, trackedScheduledEventHandlerMeta.map((x) => x.meta));
            }
        }
        const grouped = lodash_1.groupBy(eventHandlerMeta, (x) => x.discoveredMethod.parentClass.name);
        const eventHandlers = lodash_1.flatten(Object.keys(grouped).map((x) => {
...
HasuraModule = HasuraModule_1 = __decorate([
    ...
    __param(2, hasura_decorators_1.InjectHasuraConfig()),
